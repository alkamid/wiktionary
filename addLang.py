...
def main():
	...
	#1. kategoria główna
	page1 = wikipedia.Page(site, u'Kategoria:%s' % longNameCapital)
	try: page1.get()
		...
		print u'Kategoria języka "%s" już istnieje!' % shortName
	
	#2. kategoria indeks
	page2 = wikipedia.Page(site, u'Kategoria:%s (indeks)' % shortName)
	try: page2.get()
	except wikipedia.NoPage:
		...
		print u'Kategoria z indeksem języka "%s" już istnieje!' % shortName
	
	#3. szablon języka
	page3 = wikipedia.Page(site, u'Szablon:%s' % longName)
	try: page3.get()
	except wikipedia.NoPage:
		...
		print u'Szablon języka "%s" już istnieje!' % shortName
		
	#4. {{licznik}}
	page4 = wikipedia.Page(site, u'Szablon:licznik')
	if u':%s ' % shortName not in page4.get():
		zaczepienie = u'0-{{PAGESINCAT:Formy czasowników francuskich|R}}\n}}}}<noinclude>'
		...
		if s_before and s_after:
			...
			print u'Nie dodano parametru do szablonu {{licznik}}!'
	else:
		print u'Nazwa języka (%s) istnieje już  w szablonie {{licznik}}' % shortName
		
	#5. {{indeks}}
	page5 = wikipedia.Page(site, u'Szablon:indeks')
	if u'|%s=' % shortName not in page5.get():
		...
		if s_before and s_after:
			...
			print u'Nie dodano parametru do szablonu {{indeks}}!'
	else:
		print u'Nazwa języka (%s) istnieje już  w szablonie {{indeks}}' % shortName
		
	#6. {{dopracować}}
	page6 = wikipedia.Page(site, u'Szablon:dopracować')
	if u' %s=' % shortName not in page6.get():
		...
		if s_before and s_after:
			...
			print u'Nie dodano parametru do szablonu {{dopracować}}!'
	else:
		print u'Nazwa języka (%s) istnieje już w szablonie {{dopracować}}' % shortName
	...
	page71 = wikipedia.Page(site, u'Kategoria:%s w etymologii' % longNameCapital)
	try: page1.get()
	except wikipedia.NoPage:
		...
		print u'Kategoria etymologiczna języka "%s" już istnieje!' % shortName
		
	#7b. etymologia Szablon:etym/język
	
	page72 = wikipedia.Page(site, u'Szablon:etym/język')
	if u'%s\n' % shortName not in page72.get():
		if u' %s=' % (etymSkr) not in page72.get():
			...
			if s_before and s_after:
				...
				print u'Nie dodano parametru do szablonu {{etym/język}}!'
		else:
			print u'Taki skrót już istnieje w {{etym/język}}, wybierz inny'
	else:
		print u'Nazwa języka (%s) istnieje już w szablonie {{etym/język}}' % shortName	
	
	
	#9. MediaWiki:Gadget-false-blue-links.js
	page9 = wikipedia.Page(site, u'MediaWiki:Gadget-false-blue-links.js')
	if u'"%s"' % (shortName) not in page9.get():
		...
		if s_before and s_after:
			...
			page9.put(textPage9, comment=u"Dodanie języka %s" % zjezyka)
			#print textPage9
		else:
			print u'Nie dodano parametru do strony MediaWiki:Gadget-false-blue-links.js!'
	else:
		print u'Nazwa języka (%s) istnieje już na stronie MediaWiki:Gadget-false-blue-links.js' % shortName

	#9. MediaWiki:Common.js
	page10 = wikipedia.Page(site, u'MediaWiki:Common.js')
	if u'"%s"' % (shortName) not in page10.get():
		...
		if s_before and s_after:
			...
			page10.put(textPage10, comment=u"Dodanie języka %s" % zjezyka)
			#print textPage10
		else:
			print u'Nie dodano parametru do strony MediaWiki:Common.js!'
	else:
		print u'Nazwa języka (%s) istnieje już na stronie MediaWiki:Common.js' % shortName

	#8. joystick/liczone
	page8 = wikipedia.Page(site, u'Wikipedysta:Joystick/statystyka/liczone')
