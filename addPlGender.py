                    ...
                    if lang.type == 1:
                        ...
                        if lang.type == 9:
                            ...
                            for d in lang.znaczeniaDetail:
                                if u'rzeczownik' in d[0] and u'{{forma rzeczownika' not in d[0] and u'odrzeczownikowy' not in d[0] and 'rodzaj' not in d[0]:
                                    d[0] = elem[1]
                                    lang.saveChanges()
                word.push(False, u'uzupełnienie rodzaju na podstawie [[Wikipedysta:Adam/rodzaje_-_polski|listy Adama]]')
            ...
            if not len(tempList):
                ...
                if not diff:
                    try: word = Haslo(elem)
                        ...
                        if word.type == 3:
                            for lang in word.listLangs:
                                if lang.lang == u'polski':
                                    lang.pola()
                                    if lang.type != 7:
                                        change = 0
                                        for d in lang.znaczeniaDetail:
                                            temp_d = d[0]
                                            d[0] = d[0].replace(u'\'\'rzeczownik\'\'', temp)
                                            if temp_d != d[0]:
                                                change = 1
                                        if change:
                                            lang.saveChanges()
                            word.push(False, myComment=u'dodanie rodzaju; źródła: sjp.pl, Morfeusz')
...
def main():
    ...
    file = codecs.open(u'/home/adam/wikt/moje/input/rodzajenowe1.txt', encoding='utf-8')
    listAdam = []
    for line in file:
