...
class Word():
	def __init__(self, title):
		...
		self.flags = []
		self.meanings = []
		self.czescMowy = 0 # moďż˝liwoďż˝ci: 1 rzeczownik, 2 ndm (nie wiadomo), 3 przymiotnik, 4 czasownik
		self.wikitable = u''
		...
		self.added = 0 # zmienna kontrolujďż˝ca pojawienie się flagi "rďż˝cznie dodane"
		self.wymowa = u''
		self.obcy = 0
	...
	def flexTable(self, odmianaOlafa):
		...
		flagsVerb = [u'B', u'H']
		blp = 0
		blm = 1 # kontrola liczby mnogiej - domyďż˝lnie jej nie ma
		ambig = 0 # kontrola formy podstawowej - jeďż˝li moďż˝e byďż˝ wiďż˝cej niďż˝ jednďż˝ czďż˝ciďż˝ mowy, pomijamy generowanie odmiany
		type = None
		...
		if self.flex == u'nie':
			...
			if type == 'subst' and not self.morfeuszAmbig and self.morfeusz: 
				self.czescMowy = 1
				if len(self.flags):
					self.flags.append([u'1', [self.title]]) # dodanie sďż˝owa podstawowego jako flagi "1", ďż˝eby Morfeusz uwzglďż˝dniaďż˝ teďż˝ formďż˝ podst.
				
				lowerCase = self.title[0].lower() + self.title[1:] # morfeusz formďż˝ podstawowďż˝ zawsze zaczyna od maďż˝ej litery - ten trik pozwala na szukanie odmiany nazw wďż˝asnych
				temp = ''
				prev = ''
				depr = ''
				countGen = {"m1" : 0, "m2" : 0, "m3" : 0, "f" : 0, "n1" : 0, "n2" : 0, "p1" : 0, "p2" : 0, "p3" : 0} # zliczanie rodzajďż˝w zwrďż˝conych przez Morfeusza
				...
				# poniewaďż˝ mianownik, biernik i Wołacz zazwyczaj znajdujďż˝ się wďż˝rďż˝d odmiany lp, trzeba sprawdziďż˝ resztďż˝ przypadkďż˝w i na tej podstawie okreďż˝liďż˝ blm
				...
				#(w fazie rozwojowej) poniďż˝ej sprawdzanie, czy w tabelce moďż˝e znajdowaďż˝ się wďż˝tpliwa odmiana
				...
				for d in tablepl:
					if d != u'nom' and d != u'voc':
						if len(tablepl[d]) > 1:
							self.problems['kilka_form_odmiany'] = 1
					'''
				cnt = 0 #licznik pokazujďż˝cy czy znaleziono wiďż˝cej niďż˝ 1 rodzaj
				found = u''
				sum = 0
				for a in countGen:
					sum += countGen[a]
...
def morfAnalyse(word):
	...
	count = [] # tablica z zerami do wyďż˝apywania rďż˝nic w formach podstawowych
	count_first = [] # tablica z zerami do ustawiania pierwszego elementu dla danego sďż˝owa
	found = 0
	base = None
	form = u''
	text = ''
	for counter in range(numWords):
		...
		for a in analiza: #Morfeusz rozbija sďż˝owa z "ďż˝" na koďż˝cu na coďż˝ + byďż˝ (wtedy w analizie pojawia się oznakowanie "aglt"
		...
		# to jest trochďż˝ ryzykowne: jeďż˝li Morfeusz widzi niejednoznacznoďż˝ďż˝, wtedy skrypt sprawdza czy w sjp.pl to sďż˝owo jest jednoznaczne. Przykďż˝ad: "temu" w sjp.pl ma wskazuje tylko na formďż˝ podstawowďż˝ "ten"
...
def wikilink(phrase):
	...
	dontAnalyse.append(u'od') # moďż˝e byďż˝ teďż˝ od "oda"
	dontAnalyse.append(u'być') # alt: "bycie"
	dontAnalyse.append(u'lub') # alt: "lubiďż˝"
	dontAnalyse.append(u'gdzieś') # rozbija na "gdzie" i "byďż˝"
	dontAnalyse.append(u'albo') # alt: "alba"
	dontAnalyse.append(u'jak') # alt: "jaka" (okrycie wierzchnie)
	dontAnalyse.append(u'kawa') # alt: "Kawa" (?)
	dontAnalyse.append(u'sposób') # alt: "sposobiďż˝"
...
def genFromFlags(word):
	temp = u''
	prev = u''
	for a in word.flags: # szukanie rodzaju na podstawie flagi (wg maila Kolaara), byďż˝ moďż˝e przydatne gdy Morfeusz nie oznaczy jednoznacznie
...
def kwalifAndLink(string):
	#funkcja do obsďż˝ugi synonimy.ux.pl - dostaje jeden string stamtďż˝d, zamienia kwalifikatory na nasze i dodaje wikilink
...
#TODO: uwaga na liczebniki, ktďż˝re mogďż˝ mieďż˝ takďż˝ samďż˝ flagďż˝ jak przymiotniki
