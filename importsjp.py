...
class HasloSJP():
            ...
            try: ifExists = Haslo(a)
                ...
                if ifExists.type == 3:
                    for b in ifExists.listLangs:
                        if 'j�zyk polski' in b.langLong or 'termin obcy' in b.langLong or 'u�ycie mi�dzynarodowe' in b.langLong:
                            self.type = 0
    ...
    def meanings(self, noWiki=False):
        ...
        #poni�ej lista znacze� odrzucanych od razu - mo�na si� zastanowi� nad imionami
        for a in self.words:
            if not a.notRoot:
                ...
                if s_osoba or 'nazwisko' in a.tempMean or 'wie� w' in a.tempMean or 'imi� m�skie' in a.tempMean or 'imi� �e�skie' in a.tempMean:
        ...
        for a in self.words:
            ...
            for b in a.meanings:
                if b and 'np.' in b.definition:
                    self.problems['np'] = 1
                if b and 'czasownik' in a.typeText and ('[[%s]] [[si�]]' % (a.title) in b.definition or '[[%s si�]]' % (a.title) in b.definition):
...
class Word():
    def __init__(self, title, notRoot = False):
        ...
        self.wikitable = ''
        self.typeText = '{{brak|cz�� mowy}}'
        self.blpm = 0
        self.noMean = 0
        self.morfeusz = 0
        self.morfeuszType = ''
        self.morfeuszAmbig = 0
        self.added = 0 # zmienna kontroluj�ca pojawienie si� flagi "r�cznie dodane"
        self.wymowa = ''
        self.obcy = 0
    ...
    def flexTable(self, odmianaOlafa):
        ...
        ambig = 0 # kontrola formy podstawowej - je�eli mo�e by� wi�cej ni� jedn� cz�ci� mowy, pomijamy generowanie odmiany
        type = None
        ...
        if self.flex == 'nie':
            ...
            if type == 'subst' and not self.morfeuszAmbig and self.morfeusz:
                self.czescMowy = 1
                if len(self.flags):
                    self.flags.append(['1', [self.title]]) # dodanie s�owa podstawowego jako flagi "1", �eby Morfeusz uwzgl�dnia� te� form� podst.

                lowerCase = self.title[0].lower() + self.title[1:] # morfeusz form� podstawow� zawsze zaczyna od ma�ej litery - ten trik pozwala na szukanie odmiany nazw w�asnych
                temp = ''
                prev = ''
                depr = ''
                countGen = {"m1" : 0, "m2" : 0, "m3" : 0, "f" : 0, "n1" : 0, "n2" : 0, "p1" : 0, "p2" : 0, "p3" : 0} # zliczanie rodzaj�w zwr�conych przez Morfeusza
                            ...
                            for c in morfeuszTemp:
                                ...
                                if (morf[0] == 'subst' or morf[0] == 'depr') and (c[2][1] == self.title or c[2][1] == lowerCase):
                                    ...
                                    if morf[0] == 'depr':
                                        depr = c[2][0]
                                    countGen['%s' % morf[3]] += 1
                # poniewa� mianownik, biernik i wo�acz zazwyczaj znajduj� si� w�r�d odmiany lp, trzeba sprawdzi� reszt� przypadk�w i na tej podstawie okre�li� blm
                ...
                cnt = 0 # licznik pokazuj�cy czy znaleziono wi�cej ni� 1 rodzaj
                found = ''
                sum = 0
                for a in countGen:
                    sum += countGen[a]
                ...
                if cnt != 1:
                    ...
                    if found == 'm1':
                        self.typeText = '\'\'rzeczownik, rodzaj m�skoosobowy\'\''
                    elif found == 'm2':
                        self.typeText = '\'\'rzeczownik, rodzaj m�skozwierz�cy\'\''
                    elif found == 'm3':
                        self.typeText = '\'\'rzeczownik, rodzaj m�skorzeczowy\'\''
                    elif found == 'f':
                        self.typeText = '\'\'rzeczownik, rodzaj �e�ski\'\''
                    elif found == 'n2':
                ...
                if gen == 'm':
                    self.typeText = '\'\'rzeczownik, rodzaj m�ski\'\''
                elif gen == 'f':
                    self.typeText = '\'\'rzeczownik, rodzaj �e�ski\'\''
                elif gen == 'n':
                    self.typeText = '\'\'rzeczownik, rodzaj nijaki\'\''
...
def wikipage(hasloSJP, obrazki):
    ...
    if word.obcy and '�' not in firstWord.title and '�' not in firstWord.title and '�' not in firstWord.title and '�' not in firstWord.title and '�' not in firstWord.title and '�' not in firstWord.title:
        naglowek = '== %s ({{termin obcy w j�zyku polskim}}) ==' % firstWord.title
        wymowa = ' {{ortograficzny|%s}}' % firstWord.wymowa
    elif word.obcy:
        naglowek = '== %s ({{j�zyk polski}}) ==' % firstWord.title
        wymowa = ' {{ortograficzny|%s}}' % firstWord.wymowa
    else:
        naglowek = '== %s ({{j�zyk polski}}) ==' % firstWord.title
    ...
    przyklady = przyklady_szablon + ' [%s szukaj przyk�ad�w w korpusie]' % (generateConcordationsLink(firstWord.title))
...
def morfAnalyse(word):
    ...
    count = [] # tablica z zerami do wy�apywania r�nic w formach podstawowych
    ...
    for counter in range(numWords):
        ...
        for a in analiza: #Morfeusz rozbija s�owa z "�" na ko�cu na co� + by� (wtedy w analizie pojawia si� oznakowanie "aglt"
...
def wikilink(phrase):
    ...
    dontAnalyse = ['np.', 'm.in.', 'etc.', 'itd.', 'itp.', 'z', 'w', 'dzi�ki', 'co', 'po', 'pod', 'o', 'se']
    enieAnie = ('enia', 'enie', 'eniu', 'eniem', 'eniom', 'eniach', 'eniami', 'a�', 'ania', 'anie', 'aniu', 'aniem', 'aniom', 'aniach', 'aniami')
    dontAnalyse.append('od') # alt: "oda"
    dontAnalyse.append('by�') # alt: "bycie"
    dontAnalyse.append('lub') # alt: "lubi�"
    ...
    dontAnalyse.append('spos�b') # alt: "sposobi�"
    dontAnalyse.append('i��') # alt: "i�ci�
    dontAnalyse.append('dzie�') # alt: dzienia, dzienie, dzieni� (?)
...
def meanProcess(mean):
    ...
    mean = mean.replace('w j�zykoznawstwie:', '{{j�z}}')
...
def genFromFlags(word):
    temp = ''
    prev = ''
    for a in word.flags: # szukanie rodzaju na podstawie flagi (wg maila Kolaara), by� mo�e przydatne gdy Morfeusz nie oznaczy jednoznacznie
...
def generateCollocationsLink(title):
    url = 'http://www.nkjp.uni.lodz.pl/collocations_meta.jsp?query=%s**&offset=0&limit=10000&span=0&collocationalContextLeft=1&collocationalContextRight=1&minCoocFreq=5&posOfCollocate=any&sort=srodek&preserve_order=true&dummystring=�����ʳ����Ӷ�����&m_nkjpSubcorpus=balanced' % (title)
    return url

def generateConcordationsLink(title):
    url = 'http://www.nkjp.uni.lodz.pl/index_meta.jsp?query=%s**&offset=0&limit=100&span=0&sort=srodek&second_sort=srodek&groupBy=---&groupByLimit=1&preserve_order=true&dummystring=�����ʳ����Ӷ�����&m_nkjpSubcorpus=balanced' % (title)
    return url
...
def ifalreadyexists(title, existing):
    ...
    if hasloWikt.type == 3:
        for a in hasloWikt.listLangs:
            if 'j�zyk polski' in a.langLong or 'termin obcy' in a.langLong or 'u�ycie mi�dzynarodowe' in a.langLong:
                return 0
...
def inProgress(kategorie):
    ...
    re_words = re.compile(r'== (.*?) \({{j�zyk polski}}\) ==')
    listInProgress = set()
...
def derivedWordsLink(title):
    ...
    #set the number of iterations: for >5 letter words, up to 4 stems will be checked, i.e. [zabiera�, zabiera, zabier, zabie]
...
def main():
    ...
    if custom_list == 1:
        inp = codecs.open('/home/adam/wikt/moje/importsjp/r�ne.txt', encoding='utf-8')
    ...
    kategorie = []
    kategorie.append(kategoriaSlowa('custom', wordsPerPage, 'r�ne/', '\n|-\n| r�ne', 'custom'))
    ...
    inprogress = inProgress(kategorie) #KONIECZNIE W��CZY� PRZED IMPORTEM
    #inprogress = set()
...
#TODO: uwaga na liczebniki, kt�re mog� mie� tak� sam� flag� jak przymiotniki
