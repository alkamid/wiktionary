...
def wikilink(phrase):
    ...
    dontAnalyse = ['np.', 'm.in.', 'etc.', 'itd.', 'itp.', 'z', 'w', 'dzięki', 'co', 'po', 'pod', 'o']
    dontAnalyseNawias1 = []
    dontAnalyseNawias2 = []
    dontAnalyseNawiasy = []
    dontAnalysePrzecinek = []
    enieAnie = ['eń', 'enia', 'enie', 'eniu', 'eniem', 'eniom', 'eniach', 'eniami', 'ań', 'ania', 'anie', 'aniu', 'aniem', 'aniom', 'aniach', 'aniami']
    dontAnalyse.append('od') # może być też od 'oda'
    ...
    if s_przymiotnikOd:
        ...
        while (i<n):
            ...
            elif i<n-1 and (phraseTab[i+1] == 'się' or phraseTab[i+1] == 'się,' or phraseTab[i+1] == 'się.' or phraseTab[i+1] == 'się;' or phraseTab[i+1] == 'się)' or phraseTab[i+1] == 'się),' or phraseTab[i+1] == 'się);' or phraseTab[i+1] == 'się).') and morfAnalyse(word)[2] and ('inf:' in morfAnalyse(word)[2] or 'pact:' in morfAnalyse(word)[2]):
                if phraseTab[i+1][-1] == ',' or phraseTab[i+1][-1] == '.' or phraseTab[i+1][-1] == ':' or phraseTab[i+1][-1] == ')' or phraseTab[i+1][-1] == ';':
                    phraseOutput += ' %s' % (shortLink(morfAnalyse(word)[0] + ' się', word + ' się')) + phraseTab[i+1][-1]
                elif phraseTab[i+1][-2:] == '),' or phraseTab[i+1][-2:] == ').' or phraseTab[i+1][-2:] == '):' or phraseTab[i+1][-2:] == ');':
                    phraseOutput += ' %s' % (shortLink(morfAnalyse(word)[0] + ' się', word + ' się')) + phraseTab[i+1][-2:]
                else:
                    phraseOutput += ' %s' % (shortLink(morfAnalyse(word)[0] + ' się', word + ' się'))
                i+=1
            else:
