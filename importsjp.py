    ...
    if s_przymiotnikOd:
        ...
        while (i<n):
            ...
            if s_nonword_only == None:
                ...
                elif i<n-1 and 'się' in phraseTab[i+1]:
                    s_punctuation_around_sie = re.search(re_punctuation_around, phraseTab[i+1])
                    if s_punctuation_around_sie:
                        if s_punctuation_around_sie.group(2) == 'się' and morfAnalyse(s_word)[0]\
                           ...
                           and not any(('praet:sg:n' in a[0][2] or 'fin:sg:ter' in a[0][2]) for a in analyse(s_word)):
                            ...
                            #if the structure is "[verb] się [verb]" then it's hard to determine which one is reflective, so we should leave it wikified separately
                            if i<n-2:
                                ...
                                if morfAnalyse(next_word)[0] and all([any(tag + ':' in a[0][2] for tag in verb_tags) for a in analyse(next_word)]):
                                    ...
                                    analysed += ' ' + s_punctuation_around_sie.group(1) + '[[się]]' + s_punctuation_around_sie.group(3)
                                    ...
                                    i += 3
                                    continue
                            analysed = shortLink(morfAnalyse(s_word)[0] + ' się', word + ' się') + s_punctuation_around_sie.group(3)
                            i += 1
                        else:
                            analysed = shortLink(morfAnalyse(s_word)[0:2])
    ...
    if s_przymiotnikOd:
        ...
        while (i<n):
            ...
            if s_nonword_only == None:
                ...
                elif i<n-1 and 'się' in phraseTab[i+1] and '.' not in word:
                    s_punctuation_around_sie = re.search(re_punctuation_around, phraseTab[i+1])
                    if s_punctuation_around_sie:
                        if s_punctuation_around_sie.group(2) == 'się' and morfAnalyse(s_word)[0]\
                           and all([any(tag + ':' in a[0][2] for tag in verb_tags) for a in analyse(s_word)])\
                            ...
                            if i<n-1:
                                ...
                                for j in range(i+2,len(phraseTab)):
                                    ...
                                    if morfAnalyse(next_word_stripped)[0] and all([any(tag + ':' in a[0][2] for tag in verb_tags) for a in analyse(next_word_stripped)]):
                                        ...
                                        break
                                if found_verb == 0:
                                    analysed = s_punctuation_around.group(1) + shortLink(morfAnalyse(s_word)[0] + ' się', s_word + ' się') + s_punctuation_around_sie.group(3)
                                    phraseOutput += ' ' + analysed
                                    i += 2
                                    continue
                                else:
