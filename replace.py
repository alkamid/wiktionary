...
def main():
    ...
    re_forma = re.compile(u'^{{forma (czasownika|przymiotnika|rzeczownika|przysłówka|zaimka)(|\|[a-z]{2})}}$')
    re_morfem = re.compile(u'^{{morfem(|\|(eo|esperanto))(|\|(przedrostek|przedrostkowy|przyrostek|przyrostkowy))}}$')
                    ...
                    if lang.type != 2:
                        ...
                        if lang.type not in (2,3,4,5,7,11):
                            ...
                            for each in pos:
                                ...
                                if s_forma or s_morfem or each[0] == u'{{szwedzki czasownik frazowy}}':
                                    each[0] = u'\'\'%s\'\'' % (each[0])
                                    change = 1
                                    lang.saveChanges()
                if change:
                    word.push(False, myComment=u'ujednolicenie zapisu części mowy (dodanie apostrofów przy szablonach)')
