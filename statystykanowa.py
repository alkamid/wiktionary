...
class LangStats():
	...
	def addAudio(self, aud):
		self.countAudio += aud
	
	def addGraph(self, graph):
		self.countGraph += graph
...
def countAudio(input):
	...
	s_audio = re.search(wymowa, input)
	if s_audio:
		counter = 1.0
			...
			for b in haslo.listLangs:
				if b.type != 2:
					...
					if b.langLong in statList:
						...
						statList[u'%s' % b.langLong].addAudio(countAudio(b.content))
						statList[u'%s' % b.langLong].addGraph(countGraph(b.content))
						if b.type != 4:
...
def meanings(input):
	...
	if input:
		s_count = re.findall(re_count, input.tresc)
...
def refs(inputWhole, inputZrodla):
	...
	if s_refs:
		...
		if inputZrodla:
			temp = inputZrodla.tresc.replace(u'references', u'')
...
def countAll(langStat):
	...
	output = [words, len, means, audio, graph, ref, percGraph, percAudio, avgMean]
			...
			if new[a].rank[u'countWords'] == i:
				for b in old:
					if a == b:
						changes = compareTwo(old[b], new[a])
						text5 = text5 + u'\n|-\n! %d' % (i)
						text6 = text6 + u'|-\n| [[:Kategoria:%s (indeks)|%s]]\n| align="right"| %.1f\n| align="right"| %s\n| align="right"| %.0f\n| align="right"| %.1f\n| align="right"| %s\n| align="right"| %.0f\n| align="right"| %.1f\n| align="right"| %s\n| align="right"| %.0f\n| align="right"| %.0f\n' % (new[a].shortName, new[a].shortName, new[a].percGraph, changeText(changes[u'percGraph'], 1), new[a].countGraph, new[a].percAudio, changeText(changes[u'percAudio'], 1), new[a].countGraph, new[a].percRef, changeText(changes[u'percRef'], 1), new[a].countRef, new[a].countWords)
	
						
	for i in range(51):
		for a in new:
			if new[a].rank[u'avgLen'] == i:
				for b in old:
					if a == b:
						changes = compareTwo(old[b], new[a])
						text3 = text3 + u'\n|-\n! %d\n! %s' % (i, changeText(changes[u'rankAvgLen'], 0))
