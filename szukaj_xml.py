...
def main():
	...
	re_nieprzechodni_dk_ndk1 = re.compile(r'\'\'czasownik( (nie|)przechodni|)\'\' \({{ndk}} \'\'(\'(.*?)\'|brak)\'\', {{dk}} \'\'(\'(.*?)\'|brak)\'\'\)')
	re_zwrotny_dk_ndk1 = re.compile(r'\'\'czasownik zwrotny\'\' \({{ndk}} \'\'(\'(.*?) (się|sobie)\'|brak)\'\', {{dk}} \'\'(\'(.*?) (się|sobie)\'|brak)\'\'\)')

	re_nieprzechodni_dk_ndk = re.compile(r'\'\'czasownik( (nie|)przechodni|)\'\' \({{ndk}} \'\'(\'(.*?)\'|brak)\'\', {{dk}} \'\'(\'(.*?)\'|brak)\'\'\)$')
	re_zwrotny_dk_ndk = re.compile(r'\'\'czasownik zwrotny\'\' \({{ndk}} \'\'(\'(.*?) (się|sobie)\'|brak)\'\', {{dk}} \'\'(\'(.*?) (się|sobie)\'|brak)\'\'\)$')
								...
								if s1:
									...
									if group3 == h.title:
										if group5 == 'brak':
											...
											tmp = '[[%s]]' % group5
										pos[0] = '\'\'czasownik%s niedokonany\'\' ({{dk}} %s)' % (s1.group(1), tmp)
										elem.saveChanges()
										changed = 1
									elif group5 == h.title:
										if group3 == 'brak':
											...
											tmp = '[[%s]]' % group3
										pos[0] = '\'\'czasownik%s dokonany\'\' ({{dk}} %s)' % (s1.group(1), tmp)
										elem.saveChanges()
										changed = 1
								elif s2:
									...
									if s2.group(2) == h.title:
										if group4 == 'brak':
											...
											tmp = '[[%s]]' % (group4)
										pos[0] = '\'\'czasownik zwrotny niedokonany \'\'\'%s\'\'\'\'\' ({{dk}} %s)' % (group1, tmp)
										elem.saveChanges()
										changed = 1
										...
										if group1 == 'brak':
											...
											tmp = '[[%s]]' % (group1)
										pos[0] = '\'\'czasownik zwrotny dokonany \'\'\'%s\'\'\'\'\' ({{dk}} %s)' % (group4, tmp)
										elem.saveChanges()
										changed = 1
				if changed:
					h.push(False, myComment = 'ujednolicenie zapisu aspektów')
